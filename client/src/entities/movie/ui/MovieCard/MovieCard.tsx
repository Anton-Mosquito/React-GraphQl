...
import type { MovieCardProps } from '../../model/types/movie';
...
export const MovieCard = ({ movie, onCardSelect, isPreviewMode = false }: MovieCardProps) => {
  const { t } = useTranslation();
  return (
    <Card sx={{ maxWidth: 250, position: 'relative' }}>
      {!isPreviewMode && (
        <CardMenu>
          <MenuItem onClick={() => onCardSelect?.(movie)}>{t('select')}</MenuItem>
        </CardMenu>
      )}
